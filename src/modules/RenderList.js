import moment from "moment";
  ...
  export function RenderList() { 
    ...
    if((todo.toDoDate==moment().format("D/M/YYYY")) && (todo.state=='ToDo'))cell2.innerHTML=todo.toDoDate+'<br><span class="badge badge-warning"> This task is planned <br> to be done Today ! </span>';
    else cell2.innerHTML=todo.toDoDate;
    ...
    cell5.innerHTML=`<div class="custom-control custom-switch ${todo.isImportant == true ? 'bg-success' : ''}"><input type="checkbox" class="custom-control-input" id="customSwitch${todo.id}" ><label class="custom-control-label" for="customSwitch${todo.id}"></label></div>`;
